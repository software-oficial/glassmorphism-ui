...
async function verifySystem() {
    ...
    try {
        ...
        await page.evaluate(() => window.dispatcher.execute('system.toggle-theme'));
        ...
        await page.evaluate(() => window.dispatcher.execute('system.toggle-hp'));
        ...
        await page.evaluate(() => window.dispatcher.execute('system.stress-test', { intensity: 'low' }));
        const isStressing = await page.evaluate(() => window.store.getState('system').isStressing);
        console.log(`   - Stressing state: ${isStressing}`);
        await new Promise(resolve => setTimeout(resolve, 2000));
        await page.evaluate(() => window.dispatcher.execute('system.stress-test'));
        const stopped = await page.evaluate(() => !window.store.getState('system').isStressing);
        console.log(`   - Stopped state: ${stopped}`);
        // 4. Test Audit Logs
        console.log('Testing: system.audit-log...');
        const logs = await page.evaluate(() => window.dispatcher.getAuditLog());
